...
import { addPeriod, advance, upcoming } from '@/lib/recurrence';
...
describe('addPeriod', () => {
  ...
  it('handles February in a leap year', () => {
    expect(toISODate(addPeriod(new Date(2028, 0, 31), 'monthly', 31))).toBe('2028-02-29');
  });

  it('adds seven days for a weekly period', () => {
    expect(toISODate(addPeriod(new Date(2026, 8, 28), 'weekly', 28))).toBe('2026-10-05');
  });

  it('adds three months for a quarterly one, across a year boundary', () => {
    expect(toISODate(addPeriod(new Date(2026, 10, 15), 'quarterly', 15))).toBe('2027-02-15');
  });
  ...
  it('adds twelve months for a yearly one, off a leap day', () => {
    expect(toISODate(addPeriod(new Date(2028, 1, 29), 'yearly', 29))).toBe('2029-02-28');
  });
...
describe('advance', () => {
  ...
  it('leaves today where it is — the charge has not happened yet', () => {
    const result = advance('2026-09-05', 'monthly', new Date(2026, 8, 5, 14, 0));
  ...
  it('returns the input unchanged when it is not a date at all', () => {
    expect(advance('not a date', 'monthly', new Date()).next).toBe('not a date');
  });
});
...
describe('upcoming', () => {
  ...
  it('holds the anchor day across every short month in the run', () => {
    const dates = upcoming(iso(2026, 1, 31), 'monthly', new Date(2026, 0, 1), 4);
