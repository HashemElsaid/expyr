...
- title says WHAT THE THING IS, in the words a person would use, with one distinguishing detail when the document shows one: "Egyptian passport", "Toyota Corolla Mulkiya", "Marina Heights tenancy", "Driving licence", "Netflix". Name the kind of thing; the detail is what tells two of the same kind apart.
- Never put a person's name in the title, and never put a date in it. The app records who a document belongs to in a field of its own and shows it beside the title, so a name in the title is the same fact twice and the two copies drift apart the moment one is edited. "Residence Visa for AHMED SAID" is wrong; "Residence visa" is right, and if the picture shows which emirate or sponsor, that is the detail worth having instead.
